...
/**
 * Echoes the current request's traceId to the client via the {@code X-Trace-Id} response header
 * so the caller can reference it when reporting an error.
 *
 * <p>If Micrometer Tracing is active (auto-configured by {@code micrometer-tracing-bridge-brave}),
 * the traceId is already in the MDC by the time this filter runs — we use it as-is. Otherwise
 * we generate a UUID fallback and push it to MDC ourselves.
 *
